...
# --------------------------- #
# Modified to account for NAs #
# --------------------------- #

M.COMBAT <- function (dat, batch, center , mod=NULL, numCovs = NULL ) 
{
  ...
  if (!NAs) {
    ...
    B.hat = apply(dat, 1, sva:::Beta.NA, design) # inserted to account for NAs
  }
  ...
  var.batch <- apply(dat[, batch==center], 1, function(i) var(i, na.rm=T))
  
  # batch gene-wise means and standard deviations
  stand.mean=stand.sds <- matrix(NA,nrow(dat),ncol(dat))
  for( i in 1:n.batch){
    stand.mean[,batch==i] <- matrix(B.hat[i,],nrow(dat),n.batches[i])
    stand.sds[,batch==i] <- matrix(apply(dat[,batch==i],1,function(i) sd(i, na.rm=T)),nrow(dat),n.batches[i])
  }
  ...
  if (!NAs) {
    ...
    gamma.hat = apply(s.data, 1, sva:::Beta.NA, batch.design) # inserted to account for NAs
  }
  ...
  gamma.bar <- apply(gamma.hat, 1, function(i) mean(i, na.rm=T))
  t2 <- apply(gamma.hat, 1, function(i) var(i, na.rm=T))
